...
## 23.09.2026 — Hayalî bağımsız demo proje ve görsel kontrol

- Yalnız demo tenant içinde `[DEMO] Dijital üretim izleme pilotu` adlı, MOC bağlantısı olmayan bağımsız proje oluşturuldu (canlı proje id 6). Gerçek firma/tesis/müşteri verisi yoktur. Tekrar çalıştırılabilir kaynak: `moc_demo_bagimsiz_proje_2026_09_23.sql`; önce aynı işlemin `ROLLBACK` provası geçti, ardından canlıda uygulandı.
- 01.09–20.12.2026 tarihli projede beş faz, 12 görev (4 tamamlandı, 2 sürüyor, 1 incelemede, 1 bloklandı, 4 yapılacak), iki görev bağımlılığı, dört kilometre taşı, altı maliyet kalemi ve bir başlangıç baz çizgisi var. Bütçe 760.000 TRY; planlanan maliyet kalemleri 720.000 TRY, kaydedilen gider 257.000 TRY. Aylık plan Eylül–Aralık, gerçekleşen gider Eylül verisinde görünür. İlerleme kısmi görev yüzdelerini de içerdiği için 4/12 tamamlanmaya karşılık %50 görünür.
- Kanban, Gantt benzeri plan zaman çizelgesi ve maliyet grafikleri gerçek arayüzün render işlevleri ve bu canlı demo kaydıyla salt okunur yerel önizlemelerde incelendi. `project_dashboard_smoke_test.cjs` veri dosyasıyla liste/Kanban/Gantt önizlemesi oluşturup görev sütunları, 12 zaman çubuğu, dört kilometre taşı ve aylık giderleri doğrular. Canlı veritabanında proje/faz/görev/maliyet adetleri okuma sorgusuyla kontrol edildi. Gerçek oturumlu tarayıcı yazma testi bu kontrolde yapılmadı.
- Görsel kontrolde proje KPI'larının alt alta uzaması ve dar Kanban kartlarında başlık kesilmesi görüldü; KPI'lar masaüstünde dört sütuna, küçük ekranda iki sütuna alındı; kart adları satıra sarılır. Bu düzenleme veri/akış değişikliği içermez.
- Yayında HTML güncellenmiş olsa da kökteki `/qdl-version.json` eski kalabiliyordu: depo hem kökte hem `Değişiklik Yönetimi MOC/` içinde bu dosyayı tutuyor, önceki damgalama yalnız alt dizini yeniliyordu. İki dosya eşitlendi; yerel `_platform-ortak/istemci/surum-damgala.cjs` MOC için kök kopyayı da üretir. Sonraki yayında kök ve alt dizin sürümü birlikte kontrol edilmeli.
- Rakip analizinden kalan ürün farkları: Gantt üzerinde sürükleyerek tarih düzenleme ve bağımlılıkları çizgilerle gösterme; kaynak/kişi kapasitesi görünümü; yeniden kullanılabilir proje şablonları; proje risk/sorun kaydı ve portföy düzeyinde raporlama. Mevcut çizelge tarihlerden türetilen **salt görsel bir plan görünümüdür**; kullanıcı görevin tarihini formdan düzenleyebilir. Bu farklar tamamlandı diye sunulmayacak; iş değeri ve arayüz sadeliğine göre sıralanacak.
